...
void setup() {
  ...
  if (!display.begin(SSH1106_SWITCHCAPVCC, 0x3C)) 
  { 
...
void readWave() {                            
  ...
  switch (hRange) {                          
    ...
    case 4: {                                
        ...
        for (int i = 0; i < REC_LENGTH; i++) 
        {    
          waveBuff[i] = analogRead(0);       // EDCPROJECT[Gautam]
...
void startScreen() {                
  ...
  display.println(F("EDC PROJECT"));   
  display.setCursor(10, 45);         
  display.println(F("Pocket Oscilloscope by Gautam"));
  display.display();                 
  delay(5000);
  display.clearDisplay();
  display.setTextSize(1);            
